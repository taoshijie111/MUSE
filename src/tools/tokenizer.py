"""https://github.com/mikemayuare/apetokenizer/tree/main"""
...
def get_tokenizer(base_path, tokenizer_path):
    tokenizer = BartTokenizer.from_pretrained(base_path)
    unwanted_words = [i for i in tokenizer.encoder.keys()]
    important_tokens = ['<s>', '<pad>', '</s>', '<unk>']
    unwanted_words = list(set(unwanted_words).difference(set(important_tokens)))
    for word in unwanted_words:
        del tokenizer.encoder[word]
    vocab_file = os.path.join(tokenizer_path, "vocab.json")
    special_tokens_file = os.path.join(tokenizer_path, "special_tokens.json")
    with open(vocab_file, "r", encoding="utf-8") as f:
        vocabulary = json.load(f)
    with open(special_tokens_file, 'r', encoding='utf-8') as f:
        special_tokens = json.load(f)
    selfies_tokens = [v for v in vocabulary.keys() if v not in special_tokens.keys()]
    tokenizer.add_tokens('<mask>', special_tokens=True)
    tokenizer.add_tokens(selfies_tokens, special_tokens=False)
    return tokenizer
...
class APETokenizer:
    ...
    def train(
        ...
        if merge:
            while True:
                ...
                if merged_word not in vocabulary_frequency.keys() and verbose:
                    print(
        ...
        self.vocabulary = {
            ...
            **{
                word: idx
                for idx, word in enumerate(vocabulary_frequency.keys(), start=len(self.special_tokens.keys()))
            },
        }
        if verbose:
            print("\nTraining complete.")
    ...
    @property
    def vocab_size(self):
        return len(self.reverse_vocabulary)
    ...
    def convert_ids_to_tokens(self, token_ids):
        ...
        if isinstance(token_ids, list) and all(hasattr(token_id, 'item') for token_id in token_ids):
            token_ids = [int(token_id.item()) for token_id in token_ids]
